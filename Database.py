...
class DatabaseOperation():
    ...
    def __init__(self, db): #Setup db and cursor 
        self.db = db
    ...
    def SelectItemByID(self, id):
        self.mycursor.execute("SELECT * FROM Devices WHERE DeviceID = %s", (id,))
        return self.mycursor
    def SelectItemByHostname(self, Hostname):
        self.mycursor.execute("SELECT * FROM Devices WHERE Hostname = %s", (Hostname,))
        return self.mycursor.fetchone()
    
    def AddDevice(self, Device): #Takes tuple 
        insert_new_device = ("INSERT INTO Devices (Address, DeviceInfo, Hostname, Date) "
                             "VALUES (%s, %s, %s, %s)")
        self.mycursor.execute(insert_new_device, Device)
        self.db.commit()
        return
    ...
    def UpdateDevice(self, id):
        update_old_device = ("UPDATE Devices SET Date = %s "
                             "WHERE DeviceID = %s")
        self.mycursor.execute(update_old_device, (str(datetime.datetime.now()), id))
        self.db.commit()
        return
    
    def DeleteDevice(self, id):
        self.mycursor.execute("DELETE FROM Devices WHERE DeviceID = %s", (id,))
        self.db.commit()
        return
